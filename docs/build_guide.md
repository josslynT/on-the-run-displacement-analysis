...
Load the raw source files (see data/raw/) into BigQuery.
Create the lookup tables, then run the sql/intermediate/ views.
Run the sql/mart/ queries to build the mart tables.
Run the notebooks in notebooks/ (they read from the mart tables).
Build the dashboard in Looker Studio on top of the mart tables.
